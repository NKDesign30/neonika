...
describe("Neon Sessions and Activity snapshots", () => {
  it("groups Gateway runs into durable Neon session rows", async () => {
    ...
    try {
      ...
      assert.equal(snapshot.sessions[0]?.goal, "Ship Neon Core Discord parity");
      ...
      assert.match(report, /goal: Ship Neon Core Discord parity/);
...
function createSessionRun(
  ...
  return {
    ...
    request: {
      ...
      mode: "read-only",
      goal: "Ship Neon Core Discord parity",
...
async function createTempProjectRoot(): Promise<string> {
  return await mkdtemp(join(tmpdir(), "neon-core-session-activity-"));
}
