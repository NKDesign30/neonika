...
async function tempCursorPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neonika-heartbeat-daemon-test-"));
...
describe("Neon heartbeat daemon tick driver", () => {
  ...
  it("returns the empty cursor on a missing file (tolerant)", async () => {
    const cursor = await readNeonHeartbeatDaemonCursor(join(tmpdir(), "neonika-no-such-cursor.json"));
