...
describe("Neon Core product foundation", () => {
  it("defines the expected product layers", () => {
    assert.deepEqual(
      neonCoreLayers.map((layer) => layer.id),
      [
        "runtime",
        "gateway",
        "mission-control",
  ...
  it("renders the architecture around Neon Gateway and Mission Control", () => {
    ...
    assert.match(summary, /Live Neon Core APIs/);
  });
  ...
  it("keeps cutover stages explicit and ordered", () => {
    assert.deepEqual(
      neonCoreCutoverStages.map((stage) => stage.id),
