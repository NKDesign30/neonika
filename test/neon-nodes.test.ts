...
describe("Neon Nodes", () => {
  it("summarizes the local Neon Core node with read-only file-transfer policy", async () => {
    ...
    try {
      ...
      assert.equal(snapshot.localNode.displayName, "operator-mac Neon Core");
...
async function createTempProjectRoot(): Promise<string> {
  return await mkdtemp(join(tmpdir(), "neon-core-nodes-"));
}
