...
describe("Neon Peekaboo proxy", () => {
  it("resolves a project-local Unix socket path", () => {
    assert.equal(
      resolveNeonPeekabooProxySocketPath("/tmp/neon-core"),
      join("/tmp/neon-core", "state", "gateway", "peekaboo-proxy.sock")
    );
  ...
  it("proxies argv to a fixed binary over a local Unix socket", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-peekaboo-proxy-"));
  ...
  it("renders a shim that prefers loopback TCP, then socket, then the real Peekaboo binary", () => {
    const script = renderNeonPeekabooProxyShimScript({
      projectRoot: "/Users/operator/neon-core",
      socketPath: "/Users/operator/neon-core/state/gateway/peekaboo-proxy.sock",
