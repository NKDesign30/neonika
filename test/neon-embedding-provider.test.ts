...
describe("neon local embedding provider", () => {
  ...
  it("is deterministic: same text yields a bit-identical vector", () => {
    const a = embedNeonLocalText("Neon Core memory autarky");
    const b = embedNeonLocalText("Neon Core memory autarky");
  ...
  it("separates unrelated text from related text by cosine", () => {
    const base = embedNeonLocalText("Neon Core cutover plan");
    const related = embedNeonLocalText("Neon Core cutover sequence");
