...
describe("Codex app-server contract", () => {
  ...
  it("resolves Neon-owned state paths under the project root", () => {
    const paths = resolveHarnessStatePaths("/tmp/neon-core");
    const sessionKey = deriveCodexSessionKey(bindingInput);
    const bindingPath = resolveBindingPath("/tmp/neon-core", sessionKey);

    assert.equal(paths.harnessRoot, "/tmp/neon-core/state/codex-harness");
    assert.match(bindingPath, /^\/tmp\/neon-core\/state\/codex-harness\/bindings\//);
  ...
  it("round-trips thread bindings without persisting secret-looking values", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-contract-"));
    const sessionKey = deriveCodexSessionKey(bindingInput);
  ...
  it("returns undefined for corrupted binding files", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-corrupt-binding-"));
    const sessionKey = deriveCodexSessionKey(bindingInput);
