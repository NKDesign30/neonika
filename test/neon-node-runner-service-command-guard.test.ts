...
describe("Neon Node Runner Service — command injection guard", () => {
  it("blocks execution and never spawns when the persisted command is tampered after approval", async () => {
    ...
    try {
      ...
      const injectedCommand = "touch /tmp/neon-core-command-guard-pwn; echo injected";
      ...
      assert.equal(execution.command, "touch /tmp/neon-core-command-guard-pwn; echo injected");
    } finally {
...
async function createTempProjectRoot(): Promise<string> {
  return await mkdtemp(join(tmpdir(), "neon-core-node-runner-command-guard-"));
}
