...
describe("Neon merged memory provider", () => {
  it("merges primary memory with bounded local memory files and redacts snippets", async () => {
    const root = await mkdtempRoot("neonika-merged-memory-");
  ...
  it("reserves room for local memory hits when primary recall is already full", async () => {
    const root = await mkdtempRoot("neonika-merged-memory-budget-");
    const fileRoot = join(root, "agent-memory");

    try {
      await mkdir(fileRoot, { recursive: true });
      await writeFile(join(fileRoot, "chaty.md"), "Chaty knows Neonika Discord memory fusion.", "utf8");

      const provider = createMergedNeonMemoryProvider({
        primaryProvider: staticProvider([
          { source: "primary-1", text: "Neonika primary one" },
          { source: "primary-2", text: "Neonika primary two" },
          { source: "primary-3", text: "Neonika primary three" }
        ]),
        fileRoots: [{ id: "agent-memory", path: fileRoot }]
      });
      const result = await provider.search("Chaty Neonika memory", { maxHits: 3 });
  ...
  it("diversifies local memory roots so topical shared folders are not crowded out", async () => {
    const root = await mkdtempRoot("neonika-merged-memory-diverse-");
    ...
    try {
      ...
      await writeFile(
        ...
        "Chaty Neonika Discord memory Ada Lovelace analytics arena",
        "utf8"
      );
      await writeFile(
      ...
      const result = await provider.search("Chaty Neonika Discord memory Ada Lovelace analytics arena", {
        maxHits: 3
      });
