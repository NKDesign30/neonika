...
describe("neon memory db writer (gated, isolated)", () => {
  ...
  it("blocks the write when the gate is disabled", async () => {
    ...
    const result = await writeNeonMemoryDbEntry({
      dbPath,
      gate,
      input: { sourceFile: "neon-core:test", content: "should not be written", agent: "neo", category: "discoveries" }
    });
  ...
  it("writes a redacted entry that is findable via FTS", async () => {
    const result = await writeNeonMemoryDbEntry({
      ...
      gate: armedGate,
      input: {
        sourceFile: "neon-core:slice3",
        content: "Neon Core memory autarky writer slice three roundtrip",
        agent: "neo",
