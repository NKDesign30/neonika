...
// Build the control UI into the Neon Core dist tree so the gateway HTTP server
...
export default defineConfig({
  ...
  server: {
    ...
    port: 5173,
    strictPort: false,
    // Dev-only: proxy the live Neon Core API so the UI reads real data while
    // developing standalone. Production is served same-origin by the gateway,
    // so this proxy never affects the build.
    proxy: {
      "/api": {
        target: process.env.NEON_CORE_PROXY ?? "http://localhost:8797",
        changeOrigin: true,
      },
    },
  },
