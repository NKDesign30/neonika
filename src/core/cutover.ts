...
export const neonCoreCutoverStages: readonly ICutoverStage[] = [
  {
    id: "shadow",
    label: "Shadow",
    meaning: "Neon Core observes or runs manual tests only.",
  ...
  },
  {
    id: "canary",
    label: "Canary",
    meaning: "One private channel routes selected commands to Neon Core.",
    exitGate: "Rollback is one config change."
  },
  {
    id: "primary",
    label: "Primary",
    meaning: "Neon Core handles private Discord agent runs.",
...
/**
 * The cutover stages in which Neon Core is allowed to send real outbound
 * traffic. Canary opens outbound for one allowlisted channel; Primary keeps
 * outbound open once Neon Core has become the primary handler. Every other
...
export function readCutoverStageFromEnv(
  ...
  return neonCoreCutoverStages.some((stage) => stage.id === raw)
...
export function nextCutoverStage(current: TCutoverStageId): TCutoverStageId | null {
  const currentIndex = neonCoreCutoverStages.findIndex((stage) => stage.id === current);

  if (currentIndex < 0) {
    return null;
  }

  return neonCoreCutoverStages[currentIndex + 1]?.id ?? null;
}

export function renderCutoverPlan(): string {
  return neonCoreCutoverStages
