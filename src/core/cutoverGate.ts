...
const cutoverStageIds = neonCoreCutoverStages.map((stage) => stage.id);
...
function buildShadowGate(facts: ICutoverFacts, state: TNeonCutoverGateState): INeonCutoverGate {
  ...
  return createGate("shadow", state, {
    ...
    rollback: "Keep the previous runtime primary; stop Neon Core shadow listeners."
  });
}
...
function buildMirrorGate(facts: ICutoverFacts, state: TNeonCutoverGateState): INeonCutoverGate {
  ...
  return createGate("mirror", state, {
    ...
    rollback: "Keep Neon Core in shadow; discard mirror output."
  });
}
...
function createGate(
  ...
  const stage = neonCoreCutoverStages.find((entry) => entry.id === id);
