...
function recommendedDoctorAction(check: INeonDoctorCheck): string {
  ...
  switch (check.id) {
    case "node-runtime":
      return "Run Neon Core on Node >=22.19.0 <23 or >=23.11.0; Node 23.0-23.10 is blocked.";
    case "gateway":
 ...
 * missing guidance, but Neon Core agents recall through the shared `memory-search`
...
function buildCutoverCheck(
  ...
  const stage = neonCoreCutoverStages.find((entry) => entry.id === currentStage);
