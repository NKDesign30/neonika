// Copied from OpenClaw `src/auto-reply/chunk.ts` (MIT).
// See THIRD_PARTY_NOTICES.md for the full licence and attribution.
// Ported from OpenClaw src/auto-reply/chunk.ts for Discord outbound parity.
import {
  findFenceSpanAt,
  isSafeFenceBreak,
  parseFenceSpans
} from "./markdownCore/fences.js";
import {
  avoidTrailingHighSurrogateBreak,
  chunkTextByBreakResolver
} from "./markdownCore/chunk-text.js";

export type ChunkMode = "length" | "newline";

type TFenceSpan = ReturnType<typeof parseFenceSpans>[number];
...
export function chunkByParagraph(
  ...
  const pushParagraph = (paragraph: string, separatorBefore?: string): void => {
    if (!currentChunk) {
...
export function chunkMarkdownTextWithMode(text: string, limit: number, mode: ChunkMode): string[] {
  if (mode === "newline") {
    ...
    for (const chunk of paragraphChunks.flatMap((paragraphChunk) =>
      paragraphChunk.length > limit ? splitPackedFenceParagraphChunk(paragraphChunk) : paragraphChunk
    )) {
...
export function chunkMarkdownText(text: string, limit: number): string[] {
  ...
  let start = 0;
  let reopenFence: TFenceSpan | undefined;
