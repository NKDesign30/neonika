// Copied from OpenClaw `packages/markdown-core/src/ir.ts` (MIT).
...
type MarkdownToken = {
  ...
  children?: MarkdownToken[] | undefined;
...
function renderTableAsCode(state: RenderState) {
  ...
  const appendRow = (cells: TableCell[]) => {
    ...
    for (let i = 0; i < columnCount; i += 1) {
      ...
      const pad = (widths[i] ?? 0) - (cell?.text.length ?? 0);
      if (pad > 0) {
  ...
  const appendDivider = () => {
    state.text += "|";
    for (let i = 0; i < columnCount; i += 1) {
      const dashCount = Math.max(3, widths[i] ?? 0);
