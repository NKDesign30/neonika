...
/**
 * Claude Code agent backend for Neon Core.
 *
...
const TOOL_DENY_REASON =
  "Neon Core runs the Claude harness read-only; native tool execution is gated behind the cutover.";
...
function buildClaudeBaseInstructions(input: ICodexHarnessInput): string {
  return [
    "You are running inside Neon Core's Claude CLI harness.",
