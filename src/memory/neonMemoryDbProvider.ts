...
/**
 * Direct SQLite memory search for Neon Core (memory autarky).
 *
 * Neon Core owns its memory: it never depends on a companion service's running
 ...
 * point Neon Core at an existing store.
 */
 ...
 * incremented these counters on every FTS hit; the neon-core read path was
 ...
 * so Neon Core's run memory attachment and Doctor health check no longer shell out
