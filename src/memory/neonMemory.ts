...
/**
 * The autarkic default memory provider. Neon Core must outlive the previous runtime, so
 ...
 * cause) instead of silently re-coupling Neon Core to the previous runtime.
 *
 * Memory cutover (Slice K1): the primary DB path honours `NEON_MEMORY_DB_PATH`
 * (canonically the neon-core `data/semantic-memory.db`). While the primary
...
// memory host; Neon Core has no productive write CLI, so this is rebuild-native:
