...
const commands: Record<string, ICommand> = {
  status: {
    description: "Print the Neon Core product manifest.",
  ...
  "cutover-gate": {
    description: "Print evidence-based Neon Core cutover gates.",
    run: runCutoverGate
  },
  "cutover-promote": {
    description:
  ...
  "memory-maintain": {
    description:
      "Productive memory maintenance against the neon-core DB (NEON_MEMORY_DB_PATH): backup FIRST, then Ebbinghaus importance recalc + relation discovery; prune stays plan-only unless --prune-apply is passed. Gated via NEON_MEMORY_WRITE_ENABLED; hard-refuses the real v2 archive DB. Memory cutover Slice K5 — the facade replacing v2's maintenance cronjobs.",
  ...
  "commitment-hints-import-smoke": {
    description:
      "Exercise the v3 -> Neon Core commitment-hint import (Phase A migration): default-off blocks; armed path imports a sample hints file into an isolated tmp store; a second run is idempotent (skipped). Never touches production, never sends.",
...
async function runThreadSmoke(): Promise<string> {
  ...
  try {
    ...
    const thread = await startOrResumeCodexThread({
      ...
      sandbox: "read-only",
      ephemeral: true,
      baseInstructions: "Neon Core runtime smoke. Do not start a model turn."
    });
...
async function runClaudeHarnessSmoke(): Promise<string> {
  ...
  const result = await harness.run({
    prompt: "Local Neon Core Claude harness smoke",
    binding: {
      channel: "cli",
      accountId: "local",
      channelId: "terminal",
...
async function runMemoryWriteDryRun(): Promise<string> {
  const content = readTrailingArgument("Neon Core memory-write dry-run probe");
...
async function runMemoryWriteProductiveSmoke(): Promise<string> {
  const content = readTrailingArgument("Neon Core productive memory-write probe");
  const gate = resolveNeonMemoryWriteGate(process.env);
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-memory-write-smoke-"));
...
async function runMemoryDbWriteSmoke(): Promise<string> {
  const content = readTrailingArgument("Neon Core SQLite memory-write probe about fleet telemetry");
  const gate = resolveNeonMemoryDbWriteGate(process.env);
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-memory-db-write-smoke-"));
...
async function runMemoryMaintainSmoke(): Promise<string> {
  ...
  const root = await mkdtemp(join(tmpdir(), "neon-core-memory-maintain-smoke-"));
...
// neon-core DB (NEON_MEMORY_DB_PATH); the real v2 archive is hard-refused by
...
async function runMemoryMaintain(): Promise<string> {
  ...
  if (targetsRealNeonDb(dbPath)) {
    return [
      ...
      "NEON_MEMORY_DB_PATH points at the real v2 archive DB - maintenance only runs against the neon-core DB.",
      "Set NEON_MEMORY_DB_PATH to the neon-core DB (e.g. <checkout>/data/semantic-memory.db)."
    ].join("\n");
  }
...
async function runSlashInteractionShadowSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-slash-shadow-smoke-"));
...
async function runChatSendSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-chat-send-smoke-"));
...
async function runDeliveryDrainPlanSmoke(): Promise<string> {
  ...
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-delivery-drain-smoke-"));
...
async function runRunLifecycleHarnessSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-run-lifecycle-harness-"));
  ...
  const attempts: readonly IRunLifecycleCodexLiveAttempt[] = [
    {
      label: "long-text",
      prompt: [
        "Neon Core lifecycle live smoke.",
...
async function runRunLifecycleCodexLiveAttempt(
  ...
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-run-lifecycle-codex-live-"));
...
async function runDeliveryDryRunSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-delivery-smoke-"));
...
async function runDeliveryDryRunSendSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-delivery-send-smoke-"));
...
async function runCanaryOutboundLiveSmoke(): Promise<string> {
  ...
  const message = "Neon Core canary live smoke — DP-1/DP-2, allowlisted channel.";
...
async function runCanaryEmbedLiveSmoke(): Promise<string> {
  ...
  const embed: INeonDiscordEmbed = {
    title: "Neon Core canary embed smoke",
...
async function runComponentsCanaryLiveSmoke(): Promise<string> {
  ...
  const allowlist = resolveNeonCanaryChannelAllowlist(env);
  const channelId = resolveCanarySmokeChannelId(pre, allowlist);
  const content = "Neon Core canary component smoke — gated approve/reject prompt.";
...
async function runMediaCanaryLiveSmoke(): Promise<string> {
  ...
  const allowlist = resolveNeonCanaryChannelAllowlist(env);
  const channelId = resolveCanarySmokeChannelId(pre, allowlist);
  const content = "Neon Core canary media smoke — gated inline attachment.";
  const attachments: readonly TNeonDiscordMediaAttachment[] = [
    {
      name: "neon-canary-smoke.txt",
      data: new TextEncoder().encode("neon-core canary media smoke\n"),
...
async function runWebhookCanaryLiveSmoke(): Promise<string> {
  ...
  const payload: INeonWebhookPayload = {
    content: "Neon Core canary webhook smoke — gated proxy identity.",
...
async function runDeliveryDispatchSmoke(): Promise<string> {
  ...
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-delivery-dispatch-smoke-"));
...
async function runDiscordTapCanaryReplyLiveSmoke(): Promise<string> {
  ...
  try {
    ...
    const sent = await outbound.postMessage(
      ...
      [
        `<@${botUserId}> Neon Core canary reply loop smoke ${nonce}.`,
...
async function runDiscordIngressCodexLiveSmoke(): Promise<string> {
  ...
  try {
    ...
    const sent = await outbound.postMessage(
      ...
      [
        `<@${botUserId}> Neon Core ingress codex live smoke ${nonce}.`,
...
async function runDiscordIngressControlLiveSmoke(): Promise<string> {
  ...
  try {
    ...
    const sent = await outbound.postMessage(
      ...
      [
        `<@${botUserId}> Neon Core ingress control live smoke ${nonce}.`,
...
async function runChatSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-chat-smoke-"));
...
async function runSessionsSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-sessions-smoke-"));
...
async function runIndexerSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-indexer-smoke-"));
...
async function runTranscriptSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-transcript-smoke-"));
  const projectsDir = await mkdtemp(join(tmpdir(), "neon-core-transcript-projects-"));
...
async function runLiveIndexSyncSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-live-index-sync-"));
...
async function runLiveIndexDaemonSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-live-index-daemon-"));
  const transcriptProjectsDir = join(projectRoot, "claude-projects");
  const codexSessionsDir = join(projectRoot, "codex-sessions");
  const now = (): Date => new Date("2026-06-08T12:00:00.000Z");
...
async function runActivitySmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-activity-smoke-"));
...
async function runMissionControlFilterSmoke(): Promise<string> {
  const criteria = readMissionControlFilterArguments();
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-mc-filter-smoke-"));
...
async function runReplaySmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-replay-smoke-"));
...
async function runCronStoreSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-cron-store-smoke-"));
...
async function runCronCommandSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-cron-command-smoke-"));
...
function createCronCommandSmokeMessage(content: string): INeonGatewayInboundMessage {
  return {
    ...
    userDisplayName: "Operator",
    agentId: "chaty",
    workspaceRoot: "/workspace/neon-core",
    mode: "read-only",
    content,
...
async function runWorkspaceNotesSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-workspace-notes-"));
...
async function runMirrorSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-mirror-smoke-"));

  try {
    await writeNeonMirrorEvidence(projectRoot, {
      evidenceId: "mirror-smoke-evidence",
      prompt: "Compare Neon Core shadow response against legacy response.",
...
async function runCronDaemonSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-cron-daemon-smoke-"));
...
async function runCronIntentLogSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-cron-intent-smoke-"));
  const cursorPath = join(dir, "cron-daemon-cursor.json");
...
async function runCronDaemonServiceSmoke(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "neon-core-cron-service-smoke-"));
...
async function runHeartbeatDaemonSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-heartbeat-daemon-smoke-"));
...
async function runHeartbeatIntentLogSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-heartbeat-intent-smoke-"));
  const cursorPath = join(dir, "heartbeat-daemon-cursor.json");
...
async function runHeartbeatDaemonServiceSmoke(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "neon-core-heartbeat-service-smoke-"));
  const agents = heartbeatSmokeAgents();
  let clockMs = Date.parse("2026-06-02T12:00:00.000Z");
  const now = (): Date => new Date(clockMs);
...
async function runDreamTickSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-dream-tick-smoke-"));
...
async function runNodePairingTokenGateSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-token-gate-smoke-"));
...
async function runNodePairingCanaryTokenIssueSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-canary-token-smoke-"));
...
async function runNodeFileWriteSmoke(): Promise<string> {
  const content = readTrailingArgument("Neon Core gated file.write probe");
  const gate = resolveNeonNodeFileWriteGate(process.env);
  const allowlistRoot = await mkdtemp(join(tmpdir(), "neon-core-file-write-allowlist-"));
...
async function runNodeDeviceSessionHandshakeSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-session-smoke-"));
...
async function runNodeActionRequestSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-action-smoke-"));
...
async function runNodeActionApprovalSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-action-approval-smoke-"));
...
async function runNodeActionResultPreviewSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-action-result-smoke-"));
...
async function runNodeTransportSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-transport-smoke-"));
...
async function runNodeTransportResultIngestSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-transport-result-smoke-"));
...
async function runNodeTransportPollSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-transport-poll-smoke-"));
...
async function runNodeTransportResultSubmitSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-transport-submit-smoke-"));
...
async function runNodeRunnerOnce(): Promise<string> {
  const gatewayUrl = readOptionalEnv("NEON_NODE_GATEWAY_URL") ?? readOptionalEnv("NEON_CORE_GATEWAY_URL") ?? "http://127.0.0.1:8797";
...
async function runNodeRunnerOnceSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-runner-smoke-"));
...
async function runNodeRunnerLoopSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-runner-loop-smoke-"));
...
async function runNodeRunnerServiceSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-runner-service-smoke-"));
...
async function runNodeRunnerServiceActionsSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-runner-service-actions-smoke-"));
...
async function runNodeRunnerServiceCanarySmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-runner-service-canary-smoke-"));
...
async function runNodePairingSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-node-pairing-smoke-"));
...
async function runDoctorFixSmoke(): Promise<string> {
  ...
  const dir = await mkdtemp(join(tmpdir(), "neon-core-doctor-fix-smoke-"));
...
async function runMirrorRunSmoke(): Promise<string> {
  ...
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-mirror-run-smoke-"));
...
async function listenMissionControlServer(): Promise<INeonGatewayHttpServerHandle> {
  const host = process.env["NEON_CORE_HOST"] ?? "127.0.0.1";
  const preferredPort = readOptionalPort(process.env["NEON_CORE_PORT"]) ?? 8788;
...
function readOptionalPort(value: string | undefined): number | undefined {
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65_535) {
    throw new Error(`Invalid NEON_CORE_PORT: ${value}`);
  }
...
async function runWorkboardSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-workboard-smoke-"));
...
async function runWorkboardAutopilotSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-workboard-autopilot-smoke-"));
...
async function runCommitmentsSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-commitments-smoke-"));
...
async function runCommitmentHintsImportSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-commitment-hints-import-smoke-"));
  ...
  const sampleHints = {
    hints: [
      {
        id: "smoke-hint-1",
        title: "Obsidian-Memory-Vault bauen und Action-Inbox nach neon-core migrieren",
...
async function runCommitmentLifecycleSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-commitment-lifecycle-smoke-"));
  const storePath = join(dir, "commitments.jsonl");
  const nowMs = Date.parse("2026-06-03T12:00:00.000Z");
...
async function runCommitmentCaptureSmoke(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "neon-core-commitment-capture-smoke-"));
  const storePath = join(dir, "commitments.jsonl");
  const now = new Date("2026-06-03T12:00:00.000Z");
...
function createCommitmentCaptureSmokeMessage(): INeonGatewayInboundMessage {
  return {
    ...
    userDisplayName: "Operator",
    agentId: "chaty",
    workspaceRoot: "/tmp/neon-core-commitment-capture-smoke",
    mode: "read-only",
...
async function runFlowPlanReport(): Promise<string> {
  ...
  if (!flowId) {
    return "Usage: neon-core flow-plan <flowId>";
  }
...
async function runFlowsSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-flows-smoke-"));
...
async function runContextSmoke(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "neon-core-context-smoke-"));
