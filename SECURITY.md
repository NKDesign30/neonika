...
Neonika is an agent runtime. It reads messages from channels, runs coding
...
| `NEON_GATEWAY_HTTP_MUTATION_TOKEN` unset **and** `NEONIKA_HOST` set to a non-loopback address | Mutating HTTP endpoints reachable from the network without auth | Anyone who can reach the port can mutate state. Set the token, or keep the bind on loopback. |
| Node runner exec policies | Commands dispatched to paired devices | Remote command execution across machines |

If you set a non-loopback `NEONIKA_HOST`, set
...
the tools. Neonika reduces the blast radius — outbound suppressed, plugins
